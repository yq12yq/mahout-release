...
function Main( $scriptDir )
{
    ...
    Write-Log "Uninstalling Apache Mahout $FinalName"

    ###
    ### Uninstall mahout
    ###
    Uninstall "mahout" $nodeInstallRoot
}

try
{
    $scriptDir = Resolve-Path (Split-Path $MyInvocation.MyCommand.Path)
    $utilsModule = Import-Module -Name "$scriptDir\..\resources\Winpkg.Utils.psm1" -ArgumentList ("MAHOUT") -PassThru
